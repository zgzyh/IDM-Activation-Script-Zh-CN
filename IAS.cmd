...
>nul findstr /v "$" "%~nx0" && (
echo:
echo 错误：脚本存在 LF 行结束问题或脚本末尾缺少空行。
echo:
...
echo:
echo PowerShell不在运行，正在终止…
echo 如果已对 Powershell 应用限制，请撤消这些更改。
echo:
echo 检查此界面并获取帮助%mas%IAS-Help#troubleshoot
goto done2
)
...
echo:
echo [%_sid%]
echo 无法寻找到User Account SID，正在终止…
echo:
echo 检查此界面并获取帮助. %mas%IAS-Help#troubleshoot
goto done2
)
...
echo:            ___________________________________________________
echo:         
call :_color2 %_White% "             " %_Green% "在键盘上按 [1,2,3,4,5,0]选择菜单"
choice /C:123450 /N
set _erl=%errorlevel%
...
echo:
echo 正在在%SystemRoot%\Temp中创建CLSID注册表项的备份
...
echo %line%
echo:
echo     激活不适用于某些用户，IDM 可能会显示虚假的密钥提示屏幕。
echo:
...
echo 你可以在  https://www.internetdownloadmanager.com/download.html 下载IDM
goto done
)
...
echo:
echo 在 %SystemRoot%\Temp中创建注册表备份文件…
...
echo:
call :_color %Gray% "如果出现虚假的密钥屏幕，请改用“冻结试用”选项."
) else (
...
foreach ($regPath in $regPaths) {
	...
	Write-Host
	Write-Host "在 $regPath 中搜索 IDM CLSID 注册表项"
	Write-Host
...
if ($finalValues -ne $null) {
    Write-Host
    if ($lockKey -ne $null) {
        Write-Host "正在锁定 IDM CLSID 注册表值..."
    }
    if ($deleteKey -ne $null) {
        Write-Host "正在删除 IDM CLSID 注册表值..."
    }
    Write-Host
} else {
    Write-Host "未找到IDM CLSID 注册表值."
	Exit
}
...
if (($finalValues.Count -gt 20) -and ($toggle -ne $null)) {
	$lockKey = $null
	$deleteKey = 1
    Write-Host "IDM 密钥计数超过 20。立即删除它们而不是锁定它们..."
	Write-Host
}
